import d3 from 'd3';
import d3MingCloud from 'd3-cloud';
...
class MingCloud extends EventEmitter {
  ...
  getStatus() {
    return this._allInViewBox ? MingCloud.STATUS.COMPLETE : MingCloud.STATUS.INCOMPLETE;
  }
  ...
  async _updateLayout(job) {
    ...
    const mingCloudLayoutGenerator = d3MingCloud();
    mingCloudLayoutGenerator.size(job.size);
    mingCloudLayoutGenerator.padding(this._padding);
    mingCloudLayoutGenerator.rotate(ORIENTATIONS[this._orientation]);
    mingCloudLayoutGenerator.font(this._fontFamily);
    mingCloudLayoutGenerator.fontStyle(this._fontStyle);
    mingCloudLayoutGenerator.fontWeight(this._fontWeight);
    mingCloudLayoutGenerator.fontSize(tag => mapSizeToFontSize(tag.value));
    mingCloudLayoutGenerator.random(seed);
    mingCloudLayoutGenerator.spiral(this._spiral);
    mingCloudLayoutGenerator.words(job.words);
    mingCloudLayoutGenerator.text(getText);
    mingCloudLayoutGenerator.timeInterval(this._timeInterval);

    this._layoutIsUpdating = true;
    await new Promise((resolve) => {
      mingCloudLayoutGenerator.on('end', () => {
        this._layoutIsUpdating = false;
        resolve(true);
      });
      mingCloudLayoutGenerator.start();
    });
  }
...
MingCloud.STATUS = { COMPLETE: 0, INCOMPLETE: 1 };
